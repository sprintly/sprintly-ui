...
describe("GroupSort", () => {
  ...
  describe('preparing items for sort', () => {
    ...
    it('should create a hash of parents with their subitems', () => {
      const items = [genItem(2,1,1), genItem(3,1,1, null, true), genItem(3,2,4), genItem(4,2, null, null, true)];
      const result = GroupSort.createParentLookups(items);
    ...
    it('should pull all subitem parents into top-level of array', () => {
      const items = [genItem(2,1,6), genItem(4,3,8)];
      const lookups = GroupSort.createParentLookups(items);
      const result = _.map(GroupSort.prepareArrayForSort(items, lookups.parents, lookups.matched), 'number');
    ...
    it('should not add nonMatching parents from previous', () => {
      const items = [genItem(2,1,6), genItem(6,1)];
      items[1].isNonMatching = true;
      items[1].testing = true;

      const memberParents = {};
      const matchingParents = {'1:6':true};
      const result = GroupSort.prepareArrayForSort(items, memberParents, matchingParents);
    ...
    it('should convert passed-in properties to the correct item property lookup', () => {
      let actual = [];
      const stub = sinon.stub(GroupSort, 'sort').callsFake((arr, prop, dir) => {
        actual.push(prop);
        return;
      });

      const items = [genItem(1,2)];
      const expected = ['product.name', 'assigned_to.first_name',
  ...
  describe('sorting', () => {
    ...
    it("should return top-level items in descending order by number by default", () => {
      const items = [genItem(2,1), genItem(1,2)];
      const actual = GroupSort.groupSort(items);
    ...
    it("should pull the parent of the subitem and add to array if the parent is not already member", () => {
      const items = [genItem(4,1), genItem(2,1), genItem(3,1,1)];
      const actual = GroupSort.groupSort(items);
    ...
    it("should use the parents' sort order as the basis for sort", () => {
      const items = [genItem(4,1), genItem(2,1), genItem(3,1,1)];
      const actual = GroupSort.groupSort(items); // default is descending by number
      assert.deepEqual(_.map(actual, 'number'), [4, 2, 1, 3]);
    });

    it("should handle change in sort direction appropriately", () => {
      const items = [genItem(3,2), genItem(1,2), genItem(1,1), genItem(2,1)];
      const actual = GroupSort.groupSort(items, 'number', 'ascending');
      assert.deepEqual(_.map(actual, 'number'), [1, 1, 2, 3]);
    });

    it('should sort by properties other than default "number"', () => {
      const items = [genItem(1,1), genItem(2,2), genItem(1,4), genItem(4,1)];
      const actual = GroupSort.groupSort(items, 'product', 'ascending');
      const expected = ['blues clues', 'mystery wagon', 'sprintly', 'sprintly'];
    ...
    it('should handle sort on properties that are nested models', () => {
      const items = [genItem(1,1,null,'jennifer'), genItem(2,1,null,'amy'), genItem(3,2,null,'bill')];
      const actual = GroupSort.groupSort(items, 'created by', 'descending');
      const expected = ['jennifer', 'bill', 'amy'];
      _.each(actual, (obj, i) => {
    ...
    it('should sort children after parent if ascending', () => {
      const items = [genItem(5,1,1), genItem(3,1,2), genItem(4,1,1)];
      const actual = GroupSort.groupSort(items, 'number', 'ascending');
    ...
    it('should also sort children after parent if decending', () => {
      const items = [genItem(5,1,11), genItem(6,1,11), genItem(8,1,11)];
      const actual = GroupSort.groupSort(items, 'number', 'descending');
      const expected = [11, 5, 6, 8];

      assert.deepEqual(_.map(actual, 'number'), expected);
    });

    it('should respect product number when sorting and grouping subitems w/their parents', () => {
      const items = [genItem(1,15,5), genItem(1,14,5), genItem(2,15,5), genItem(2,14,5)];
      const actual = GroupSort.groupSort(items, 'number', 'ascending');
      const expected = [14, 14, 14, 15, 15, 15]; // one each for parent and subitem
    ...
    it('should order by number (ascending) within parent-sorted sets', () => {
      const items = [genItem(1,15,5), genItem(1,14,5), genItem(3,14,5), genItem(2,15,5)];
      const actual = GroupSort.groupSort(items, 'number', 'descending');
      const expected = [[5,15], [1,15], [2,15], [5,14], [1,14], [3,14]]; // one each for parent and subitem
