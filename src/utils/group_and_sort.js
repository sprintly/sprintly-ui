...
export default class GroupSort {
  static groupSort(jsonArray, property, direction) {
    /*
    ...
    const lookups = this.createParentLookups(jsonArray);
    const parents = lookups.parents;
    const matched = lookups.matched;
  
    const preparedItems = this.prepareArrayForSort(jsonArray, parents, matched);
  ...
  static createParentLookups(arr) {
    /*
  ...
  static prepareArrayForSort(arr, memberParents, matchingParents) {
    /*
    ...
    _.each(arr, (item) => {
      const parentId = item.parent ? item.product.id + ':' + item.parent.number : null;
  ...
  static sort(processedJson, property) {
    /*
    ...
    return _.sortBy(processedJson, (item) => {
      let itemProperty = this.parentPreferred(item, property);
  ...
  static reverseSort(sortedArray) {
    /*
  ...
  static parentPreferred(item, prop) {
